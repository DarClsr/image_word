...
> 版本：v1.0  
> 更新日期：2026-01-25  
> 项目定位：为图文生成小程序提供安全、可靠、高性能的后端服务，支持用户认证、作品管理、任务调度、模型调用等核心功能。
...
| **NestJS** | ^10.x | 企业级 Node.js 框架，模块化、依赖注入 |
| **TypeScript** | ^5.x | 类型安全 |
| **Prisma** | ^5.x | 下一代 ORM，类型安全、自动迁移 |

### 1.2 数据存储

| 技术 | 用途 |
|------|------|
| **PostgreSQL** | 主数据库，存储用户、作品、分类等业务数据 |
| **Redis** | 缓存、会话、任务队列、分布式锁 |
| **MinIO / 腾讯 COS** | 对象存储，存储生成图片 |
...
| **bcrypt** | 密码哈希 |
| **helmet** | HTTP 安全头 |
| **class-validator** | DTO 验证（配合 Zod） |
| **winston** | 日志记录 |
| **dayjs** | 日期处理 |

### 1.4 类型验证（Zod）

后端同样使用 Zod 进行运行时类型验证，确保数据安全：
...
export const CreateCategoryDto = z.object({
  ...
  code: z.string().regex(/^[a-z0-9_]+$/, '编码格式错误'),
...
│                              NestJS Application                          │
├─────────────┬─────────────┬─────────────┬─────────────┬────────────────┤
│   Auth      │   User      │   Category  │   Works     │   Task         │
│   Module    │   Module    │   Module    │   Module    │   Module       │
├─────────────┼─────────────┼─────────────┼─────────────┼────────────────┤
│ · 微信登录   │ · 用户管理   │ · 分类管理   │ · 作品管理   │ · 任务队列     │
│ · JWT 认证  │ · 会员管理   │ · 风格管理   │ · 审核管理   │ · 生成调度     │
│ · 权限控制   │ · 额度管理   │ · 模型管理   │ · 下载统计   │ · 状态追踪     │
└─────────────┴─────────────┴─────────────┴─────────────┴────────────────┘
                                    │
                    ┌───────────────┼───────────────┐
                    ▼               ▼               ▼
...
│ id          │──┐    │ id          │    ┌──│ id          │
│ openid      │  │    │ userId      │◄───┘  │ name        │
│ unionid     │  │    │ styleId     │◄──────│ code        │
│ nickname    │  └───►│ modelId     │◄──────│ type        │
│ avatar      │       │ prompt      │       │ icon        │
│ phone       │       │ imageUrl    │       │ cover       │
│ gender      │       │ status      │       │ parentId    │
│ memberType  │       │ isPublic    │       │ sort        │
│ quota       │       │ viewCount   │       │ status      │
...
└─────────────┘
        ...
        ▼
┌─────────────┐       ┌─────────────┐       ┌─────────────┐
│  AdminUser  │       │    Task     │       │  AuditLog   │
├─────────────┤       ├─────────────┤       ├─────────────┤
│ id          │       │ id          │       │ id          │
│ username    │       │ userId      │       │ adminId     │
│ password    │       │ type        │       │ action      │
│ salt        │       │ status      │       │ module      │
│ role        │       │ params      │       │ targetId    │
│ permissions │       │ result      │       │ oldValue    │
│ status      │       │ retryCount  │       │ newValue    │
│ lastLoginAt │       │ errorMsg    │       │ ip          │
...
| 模块 | 方法 | 路径 | 说明 |
|------|------|------|------|
| **认证** | POST | `/api/client/auth/wechat-login` | 微信登录 |
| | POST | `/api/client/auth/refresh` | 刷新 Token |
| | GET | `/api/client/auth/profile` | 获取用户信息 |
| **分类** | GET | `/api/client/category/styles` | 风格列表 |
| | GET | `/api/client/category/models` | 模型列表 |
| **作品** | POST | `/api/client/works` | 创建作品（提交生成任务） |
| | GET | `/api/client/works` | 我的作品列表 |
| | GET | `/api/client/works/:id` | 作品详情 |
| | DELETE | `/api/client/works/:id` | 删除作品 |
| | GET | `/api/client/works/public` | 公开作品广场 |
| **任务** | GET | `/api/client/task/:taskId` | 查询任务状态 |
| | DELETE | `/api/client/task/:taskId` | 取消任务 |
...
| **面板** | GET | `/api/admin/dashboard/overview` | 概览数据 |
| | GET | `/api/admin/dashboard/trend` | 趋势数据 |
| **分类** | GET | `/api/admin/category` | 分类列表 |
| | POST | `/api/admin/category` | 新增分类 |
| | PUT | `/api/admin/category/:id` | 编辑分类 |
| | DELETE | `/api/admin/category/:id` | 删除分类 |
| **作品** | GET | `/api/admin/works` | 作品列表 |
| | PUT | `/api/admin/works/:id/audit` | 审核作品 |
| | PUT | `/api/admin/works/batch-audit` | 批量审核 |
| | DELETE | `/api/admin/works/:id` | 删除作品 |
| **用户** | GET | `/api/admin/user` | 用户列表 |
| | PUT | `/api/admin/user/:id/ban` | 封禁 |
| | PUT | `/api/admin/user/:id/unban` | 解封 |
| | PUT | `/api/admin/user/:id/quota` | 调整额度 |
| **日志** | GET | `/api/admin/audit-log` | 操作日志 |
...
本系统支持两个客户端，采用不同的认证机制和 Token 校验策略：

| 端 | 路由前缀 | 认证方式 | Token 类型 | 过期时间 | 权限模型 |
|---|---|---|---|---|---|
| **小程序端** | `/api/client/*` | 微信登录 | `client_token` | 7天 | 用户角色（普通/会员） |
| **后台管理端** | `/api/admin/*` | 账号密码 | `admin_token` | 2小时 | RBAC 角色权限 |
...
```
              ...
              ▼                          ▼                          ▼
    ┌─────────────────┐      ┌─────────────────┐      ┌─────────────────┐
    │  /api/client/*  │      │  /api/admin/*   │      │  /api/public/*  │
    │   小程序端接口    │      │   后台管理接口    │      │    公开接口      │
    └────────┬────────┘      └────────┬────────┘      └─────────────────┘
             │                        │
             ▼                        ▼
    ┌─────────────────┐      ┌─────────────────┐
    │ ClientAuthGuard │      │ AdminAuthGuard  │
    │   微信登录校验    │      │  账号密码校验     │
...
export const JwtConfig = {
  // 小程序端配置
  client: {
    secret: process.env.JWT_CLIENT_SECRET,       // 独立密钥
    accessExpiresIn: '7d',                       // 访问令牌 7天
    refreshExpiresIn: '30d',                     // 刷新令牌 30天
    issuer: 'image-word-client',
  },
  
  // 后台管理端配置
  admin: {
    secret: process.env.JWT_ADMIN_SECRET,        // 独立密钥（与小程序端不同）
    accessExpiresIn: '2h',                       // 访问令牌 2小时（更严格）
    refreshExpiresIn: '7d',                      // 刷新令牌 7天
    issuer: 'image-word-admin',
  },
};
```
...
```typescript
// src/modules/auth/services/client-auth.service.ts
...
export class ClientAuthService {
   ...
   * 生成小程序端 Token
   */
  async generateClientTokens(user: User) {
    const payload: Omit<ClientTokenPayload, 'iat' | 'exp'> = {
      sub: user.id,
      type: 'client',
      tokenType: 'access',
      openid: user.openid,
    ...
    // 存储 Token 到 Redis（支持主动失效）
    await this.redis.set(
      `client_token:${user.id}`,
      refreshToken,
      'EX',
      30 * 24 * 60 * 60
    );
  ...
  async validateClientToken(token: string): Promise<ClientTokenPayload> {
    try {
      ...
      const user = await this.prisma.user.findUnique({
        ...
        select: { id: true, status: true, openid: true },
      });
...
export class AdminAuthService {
  ...
  async login(username: string, password: string, ip: string) {
    ...
    if (failCount && parseInt(failCount) >= 5) {
      throw new TooManyRequestsException('登录失败次数过多，请 30 分钟后重试');
    }
   ...
   * 生成后台管理端 Token
   */
  async generateAdminTokens(admin: AdminUser) {
    ...
    const [accessToken, refreshToken] = await Promise.all([
      this.jwtService.signAsync(
        { ...payload, tokenType: 'access' },
        {
          secret: process.env.JWT_ADMIN_SECRET,  // 独立密钥
          expiresIn: '2h',                       // 更短的过期时间
          issuer: 'image-word-admin',
        }
      ),
      this.jwtService.signAsync(
    ...
    // 存储 Token（单点登录：新登录会踢掉旧登录）
    await this.redis.set(
      `admin_token:${admin.id}`,
      refreshToken,
      'EX',
      7 * 24 * 60 * 60
    );
   ...
   * 验证后台管理端 Token
   */
  async validateAdminToken(token: string): Promise<AdminTokenPayload> {
    try {
      const payload = await this.jwtService.verifyAsync(token, {
        secret: process.env.JWT_ADMIN_SECRET,
        issuer: 'image-word-admin',
  ...
  private async incrementLoginFail(key: string) {
    ...
    if (count === 1) {
      await this.redis.expire(key, 30 * 60); // 30分钟过期
    }
  }
}
```
...
export class ClientAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // 验证 Token 并获取用户信息
    const payload = await this.clientAuthService.validateClientToken(token);
...
export class AdminAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // 验证 Token
    const payload = await this.adminAuthService.validateAdminToken(token);
...
@Module({
  imports: [
    ...
    RouterModule.register([
      {
        path: 'api/client',        // 小程序端前缀
        module: ClientModule,
        children: [
          { path: 'auth', module: ClientAuthModule },
          { path: 'category', module: ClientCategoryModule },
          { path: 'works', module: ClientWorksModule },
          { path: 'task', module: ClientTaskModule },
          { path: 'user', module: ClientUserModule },
        ],
      },
      {
        path: 'api/admin',         // 后台管理端前缀
        module: AdminModule,
        children: [
          { path: 'auth', module: AdminAuthModule },
          { path: 'dashboard', module: AdminDashboardModule },
          { path: 'category', module: AdminCategoryModule },
          { path: 'works', module: AdminWorksModule },
          { path: 'user', module: AdminUserModule },
          { path: 'system', module: AdminSystemModule },
        ],
      },
    ]),
  ],
...
/**
 * 后台管理端模块 - 统一应用 AdminAuthGuard + PermissionGuard
 */
@Module({
  providers: [
    {
...
```typescript
// 小程序端接口示例
// src/modules/client/works/works.controller.ts
@Controller()  // 路由前缀由 RouterModule 配置
export class ClientWorksController {
  @Get()
  async list(@CurrentUser() user: User) {
    // user 来自 ClientAuthGuard 注入
    return this.worksService.findByUser(user.id);
  }
...
```typescript
// 后台管理端接口示例
// src/modules/admin/works/works.controller.ts
@Controller()
export class AdminWorksController {
  @Get()
  @Permissions('works:list')  // 需要 works:list 权限
  async list(@Query() query: QueryWorksInput) {
    return this.worksService.findAll(query);
  }

  @Put(':id/audit')
  @Permissions('works:audit')  // 需要 works:audit 权限
  async audit(
    @Param('id') id: number,
    @Body() dto: AuditWorksInput,
    @CurrentAdmin() admin: AdminUser,
  ) {
    return this.worksService.audit(id, dto, admin);
  }

  @Delete(':id')
  @Permissions('works:delete')
  async remove(@Param('id') id: number, @CurrentAdmin() admin: AdminUser) {
    return this.worksService.remove(id, admin);
  }
}
```
...
# 小程序端 JWT 密钥（至少32位）
JWT_CLIENT_SECRET=your-client-secret-key-at-least-32-characters

# 后台管理端 JWT 密钥（与小程序端不同）
JWT_ADMIN_SECRET=your-admin-secret-key-at-least-32-characters
...
| 安全措施 | 小程序端 | 后台管理端 |
|---------|---------|-----------|
| **认证方式** | 微信 code 换 Token | 账号 + 密码 |
| **密钥** | `JWT_CLIENT_SECRET` | `JWT_ADMIN_SECRET`（独立） |
| **Token 过期** | 7天 | 2小时（更严格） |
| **刷新令牌** | 30天 | 7天 |
| **登录限制** | 无 | 5次失败锁定30分钟 |
| **单点登录** | 否（多设备） | 是（新登录踢旧登录） |
| **权限校验** | 用户状态检查 | RBAC 权限校验 |
| **操作审计** | 关键操作记录 | 全量操作记录 |
| **IP 白名单** | 否 | 可选配置 |
...
export class AuthService {
  // Token 配置
  private readonly ACCESS_TOKEN_EXPIRES = '15m';   // 访问令牌15分钟
  private readonly REFRESH_TOKEN_EXPIRES = '7d';   // 刷新令牌7天
  ...
  async generateTokens(userId: number, role?: string) {
    ...
    // 存储 refreshToken 到 Redis（支持主动失效）
    await this.redis.set(
...
export class StorageService {
  ...
  async uploadImage(file: Express.Multer.File, userId: number): Promise<string> {
    ...
    if (!isValid) {
      throw new BadRequestException('文件内容无效');
    }
...
export class WorksService {
  async create(dto: CreateWorksInput, user: User) {
    ...
    if (user.usedQuota >= user.totalQuota) {
      throw new BadRequestException('生成额度不足');
    }
...
// 3. 数据库层 - 约束验证
// Prisma schema 中的约束：@unique, @default, 关系约束等
```
...
export class DataMonitorService {
  ...
  async checkDataAnomalies() {
    ...
    if (quotaAnomalies.length > 0) {
      this.logger.error('数据异常：发现额度超支用户', {
  ...
  imports: [
    BullModule.registerQueue({
      ...
      defaultJobOptions: {
        ...
        removeOnFail: 1000,             // 保留最近1000个失败任务
      },
    }),
  ],
})
...
export class TaskProcessor {
  ...
  async handleGeneration(job: Job<z.infer<typeof GenerationJobDataSchema>>) {
    ...
    try {
      ...
      this.logger.log(`任务完成: ${taskId}, 作品ID: ${works.id}`);
  ...
  async onFailed(job: Job, error: Error) {
    this.logger.error(`任务最终失败: ${job.data.taskId}`, {
      error: error.message,
